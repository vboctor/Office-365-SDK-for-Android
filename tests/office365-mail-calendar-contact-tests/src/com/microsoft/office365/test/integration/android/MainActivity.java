...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		AndroidTestPlatformContext testPlatformContext = new AndroidTestPlatformContext(
				this);
		...
		TestCaseAdapter testCaseAdapter = new TestCaseAdapter(this,
				R.layout.row_list_test_case);
		mTestCaseList.setAdapter(testCaseAdapter);

		mTestGroupSpinner = (Spinner) findViewById(R.id.testGroupSpinner);

		ArrayAdapter<TestGroup> testGroupAdapter = new ArrayAdapter<TestGroup>(
	...
	private void refreshTestGroupsAndLog() {
		...
		ArrayAdapter<TestGroup> adapter = (ArrayAdapter<TestGroup>) mTestGroupSpinner
				.getAdapter();
		adapter.clear();
		adapter.add(new MailTests("MailTests"));
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_view_log:
			...
			String logContent = TextUtils.htmlEncode(mLog.toString()).replace(
					"\n", "<br />");
			String logHtml = "<html><body><pre>" + logContent
					+ "</pre></body></html>";
			webView.loadData(logHtml, "text/html", "utf-8");

			logDialogBuilder.setPositiveButton("Copy",
	...
	private void fillTestList(List<TestCase> testCases) {
		TestCaseAdapter testCaseAdapter = (TestCaseAdapter) mTestCaseList
				.getAdapter();
	...
	private void runTests() {
		...
		group.runTests(new TestExecutionCallback() {
			...
			public void onTestStart(TestCase test) {
				TestCaseAdapter adapter = (TestCaseAdapter) mTestCaseList
						.getAdapter();
			...
			public void onTestGroupComplete(TestGroup group,
				...
				log("TEST GROUP COMPLETED", group.getName() + " - "
			...
			public void onTestComplete(TestCase test, TestResult result) {
				...
				final TestCaseAdapter adapter = (TestCaseAdapter) mTestCaseList
						.getAdapter();
				...
				log("TEST COMPLETED", test.getName() + " - "
						+ result.getStatus().toString() + " - Ex: " + exMessage);
				logSeparator();
			}
		});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		AndroidTestPlatformContext.context.onActivityResult(requestCode,
				resultCode, data);
	}
